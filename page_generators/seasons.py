            ...
            for tid in t_ids:
                ...
                for rid in sorted_round_ids:
                    ...
                    for group_id, group_data in groups.items():
                        players_list = []
                        for pid, pdata in group_data['players'].items():
                            players_list.append({'id': pid, 'points': pdata['total_points']})
                        
                        # Sort by points descending
                        players_list.sort(key=lambda x: x['points'], reverse=True)
                        
                        # Identify top 2 (and ties)
                        leaders = set()
                        if players_list:
                            # 1st place
                            first_points = players_list[0]['points']
                            for p in players_list:
                                if p['points'] == first_points:
                                    leaders.add(p['id'])
                            
                            # If fewer than 2 leaders (i.e. clear 1st), check 2nd place
                            if len(leaders) < 2 and len(players_list) > 1:
                                # Find next highest score
                                second_points = -1
                                for p in players_list:
                                    if p['points'] < first_points:
                                        second_points = p['points']
                                        break
                                
                                if second_points != -1:
                                    for p in players_list:
                                        if p['points'] == second_points:
                                            leaders.add(p['id'])
                        
                        # Mark leaders in the player data
                        for pid in group_data['players']:
                            if pid in leaders:
                                group_data['players'][pid]['is_leader'] = True
                            else:
